...
// 一定进行容器树上的数据绑定
@connect(state=>({
    dataBoatGroupValid: state.inputBoat.dataBoatGroupValid,
}))
export default class BoatListTwo extends PureComponent {
    ...
    render(){
        ...
        if (this.props.dataBoatGroupValid){
            console.log(this.props.dataBoatGroupValid, "this.props.dataBoatGroupValid")
            this.props.dataBoatGroupValid.forEach((data, index) => {
                dataSource.push({
                    key: index,
